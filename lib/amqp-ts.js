        ...
        this.initialized = new Promise(function (resolve, reject) {
            _this._connection.initialized.then(function () {
                _this._connection._connection.createConfirmChannel(function (err, channel) {
                    /* istanbul ignore if */
                    if (err) {
                        exports.log.log("error", "Failed on channel creation: " + err.message, { module: "amqp-ts" });
                        reject(err);
                    }
                    else {
        ...
        return new Promise(function (resolve, reject) {
            _this._consumerInitialized.then(function () {
                _this._channel.cancel(_this._consumerTag, function (err, ok) {
                    /* istanbul ignore if */
                    if (err) {
                        exports.log.log("error", "Channel failed on cancelling: " + err.message, { module: "amqp-ts" });
                        reject(err);
                    }
                    else {
                        exports.log.log("info", "Channel canceled.", { module: "amqp-ts" });
