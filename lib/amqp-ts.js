...
exports.log = logger_1.logger.jsonLogger(process.env.AMQPTS_LOGLEVEL || "error");
...
var Connection = /** @class */ (function (_super) {
    ...
    Connection.prototype.rebuildConnection = function () {
        ...
        if (this._rebuilding) { // only one rebuild process can be active at any time
            exports.log.debug("Connection rebuild already in progress, joining active rebuild attempt.", { module: "amqp-ts" });
            ...
            _this.tryToConnect(_this, 0, function (err) {
                ...
                if (err) {
                    ...
                    if (_this.connectedBefore) {
                        exports.log.warn("Connection re-established", { module: "amqp-ts" });
                        _this.emit("re_established_connection");
                    }
                    else {
                        exports.log.info("Connection established.", { module: "amqp-ts" });
        ...
        this.initialized.catch(function (error) {
            exports.log.warn("Error creating connection!", { module: "amqp-ts", error: error });
            _this.emit("error_connection", error);
            //throw (err);
        });
        return this.initialized;
    };
    Connection.prototype.tryToConnect = function (thisConnection, retry, callback) {
        var _this = this;
        AmqpLib.connect(thisConnection.url, thisConnection.socketOptions, function (error, connection) {
            /* istanbul ignore if */
            if (error) {
                ...
                exports.log.warn("Connection failed.", { module: "amqp-ts", error: error });
                _this._retry = retry;
                if (thisConnection.reconnectStrategy.retries === 0 || thisConnection.reconnectStrategy.retries > retry) {
                    exports.log.warn("Connection retry " + (retry + 1) + " in " + thisConnection.reconnectStrategy.interval + "ms", { module: "amqp-ts" });
                    ...
                    exports.log.warn("Connection failed, exiting: No connection retries left (retry " + retry + ").", { module: "amqp-ts" });
                    callback(error);
                }
            }
            else {
                var restart = function (error) {
                    exports.log.debug("Connection error occurred.", { module: "amqp-ts", error: error });
    ...
    Connection.prototype._rebuildAll = function (error) {
        var _this = this;
        if (this.rebuildPromise) {
            return this.rebuildPromise;
        }
        this.rebuildPromise = Promise.resolve().then(function () {
            exports.log.warn("Connection error: " + error.message, { module: "amqp-ts", error: error });
            exports.log.debug("Rebuilding connection NOW.", { module: "amqp-ts" });
            _this.rebuildConnection();
            ...
            for (var exchangeId in _this._exchanges) {
                ...
                exports.log.debug("Re-initialize Exchange '" + exchange._name + "'.", { module: "amqp-ts" });
                exchange._initialize();
            }
            for (var queueId in _this._queues) {
                var queue = _this._queues[queueId];
                var consumer = queue._consumer;
                exports.log.debug("Re-initialize queue '" + queue._name + "'.", { module: "amqp-ts" });
                queue._initialize();
                if (consumer) {
                    exports.log.debug("Re-initialize consumer for queue '" + queue._name + "'.", { module: "amqp-ts" });
                    queue._initializeConsumer();
                }
            }
            for (var bindingId in _this._bindings) {
                var binding = _this._bindings[bindingId];
                exports.log.debug("Re-initialize binding from '" + binding._source._name + "' to '" +
                    binding._destination._name + "'.", { module: "amqp-ts" });
                binding._initialize();
            }
            return new Promise(function (resolve, reject) {
                _this.completeConfiguration().then(function () {
                    exports.log.debug("Rebuild success.", { module: "amqp-ts" });
                    resolve(null);
                }, /* istanbul ignore next */ function (rejectReason) {
                    exports.log.debug("Rebuild failed.", { module: "amqp-ts" });
                    reject(rejectReason);
                });
            });
        })
            .then(function () {
            _this.rebuildPromise = undefined;
        })
            .catch(function (e) {
            _this.rebuildPromise = undefined;
            throw e;
    ...
    Message.prototype.sendTo = function (destination, routingKey) {
        ...
        var sendMessage = function () {
            return new Promise(function (resolve, reject) {
                ...
                connection._rebuildAll(error).then(function () {
    ...
    Exchange.prototype._initialize = function () {
        ...
        this.initialized = new Promise(function (resolve, reject) {
            _this._connection.initialized.then(function () {
                _this._connection._connection.createConfirmChannel(function (error, channel) {
                    /* istanbul ignore if */
                    if (error) {
                        exports.log.error("Failed on channel creation: " + error.message, { module: "amqp-ts", error: error });
                        reject(error);
                    }
                    else {
                        exports.log.info("Channel created.", { module: "amqp-ts" });
                        _this._channel = channel;
                        var callback = function (err, ok) {
                            /* istanbul ignore if */
                            if (err) {
                                exports.log.error("Failed to create exchange '" + _this._name + "'.", { module: "amqp-ts", error: err });
                ...
                exports.log.warn("Channel failure, error caused during connection!", { module: "amqp-ts", error: error });
            });
        });
                        ...
                        if (err) {
                            ...
                            _this._channel.close(function (err) {
                                ...
                                if (err) {
                                    ...
                                    exports.log.info("Channel closed.", { module: "amqp-ts" });
                ...
                _this._connection._connection.createConfirmChannel(function (error, channel) {
                    ...
                    if (error) {
                        ...
                        var callback = function (err, ok) {
                            /* istanbul ignore if */
                            if (err) {
                                exports.log.error("Failed to create queue '" + _this._name + "'.", { module: "amqp-ts", error: err });
        ...
        var processedMsgConsumer = function (msg) {
            try {
                ...
                Promise.resolve(result).then(function (resultValue) {
                    ...
                    exports.log.error("Queue.onMessage RPC promise returned error: " + error.message, { module: "amqp-ts", error: error });
                });
            }
            catch (error) {
                /* istanbul ignore next */
                exports.log.error("Queue.onMessage consumer function returned error: " + error.message, { module: "amqp-ts", error: error });
            }
        };
    ...
    Queue.prototype.stopConsumer = function () {
        ...
        return new Promise(function (resolve, reject) {
            _this._consumerInitialized.then(function () {
                _this._channel.cancel(_this._consumerTag, function (error, ok) {
                    /* istanbul ignore if */
                    if (error) {
                        exports.log.error("Channel failed on cancelling: " + error.message, { module: "amqp-ts", error: error });
                        reject(error);
                    }
                    else {
                        exports.log.info("Channel canceled.", { module: "amqp-ts" });
        ...
        this.initialized = new Promise(function (resolve, reject) {
            if (_this._destination instanceof Queue) {
                ...
                queue.initialized.then(function () {
                    queue._channel.bindQueue(_this._destination._name, _this._source._name, _this._pattern, _this._args, function (error, ok) {
                        /* istanbul ignore if */
                        if (error) {
                            exports.log.error("Failed to create queue binding (" +
                                _this._source._name + "->" + _this._destination._name + ")", { module: "amqp-ts", error: error });
                ...
                exchange.initialized.then(function () {
                    exchange._channel.bindExchange(_this._destination._name, _this._source._name, _this._pattern, _this._args, function (error, ok) {
                        /* istanbul ignore if */
                        if (error) {
                            exports.log.error("Failed to create exchange binding (" +
                                _this._source._name + "->" + _this._destination._name + ")", { module: "amqp-ts", error: error });
                            delete _this._destination._connection._bindings[Binding.id(_this._destination, _this._source, _this._pattern)];
                            reject(error);
                        }
